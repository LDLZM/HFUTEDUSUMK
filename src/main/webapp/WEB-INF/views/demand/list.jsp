...
<div class="container">
  <div class="page-header">
    ...
    <div class="action-btn-group">
      ...
      <a href="${pageContext.request.contextPath}/demand/add" class="action-btn">添加需求</a>

    </div>

    <!-- 搜索和过滤表单 -->
    <form action="${pageContext.request.contextPath}/demand/list" method="get">
  ...
  <table>
    ...
    <c:forEach items="${demands}" var="demand">
      <tr>
        ...
        <td class="action">
          <!-- 根据状态决定是否显示编辑链接 -->
          <c:if test="${demand.is_closed == 0}">
            <a href="${pageContext.request.contextPath}/demand/edit/${demand.demandId}" class="action-link">编辑</a>
          </c:if>
          <c:if test="${demand.is_closed != 0}">
            <span class="disabled-link">编辑</span>
          </c:if>

          <!-- 根据状态决定是否显示删除链接 -->
          <c:if test="${demand.is_closed == 0}">
            <a href="${pageContext.request.contextPath}/demand/delete/${demand.demandId}" class="action-link">删除</a>
          </c:if>
          <c:if test="${demand.is_closed != 0}">
